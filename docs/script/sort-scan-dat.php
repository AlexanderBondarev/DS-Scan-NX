...
//  if($mE[$i]['E']>$mE[$i-1]['E'] && $mE[$i]['E']>$mE[$i+1]['E'] && $mE[$i]['E']>$mE[$i-2]['E'] && $mE[$i]['E']>$mE[$i+2]['E'] && $mE[$i]['E']>$mE[$i-3]['E'] && $mE[$i]['E']>$mE[$i+3]['E']) 
  if($mE[$i]['E']>$mE[$i-1]['E'] && $mE[$i]['E']>$mE[$i+1]['E'] && $mE[$i]['E']>$mE[$i-2]['E'] && $mE[$i]['E']>$mE[$i+2]['E'] && $mE[$i]['E']>$mE[$i-3]['E'] && $mE[$i]['E']>$mE[$i+3]['E']) { 
   if(count($Extremum)>0) {
    $last = end($Extremum);
    $last_i = $last['index'];
    if(abs($mE[$last_i]['E']-$mE[$i]['E'])<2.0) continue;
    if($last['type']=='<font color=red>max</font>') continue;
   }
...
//  if($mE[$i]['E']<$mE[$i-1]['E'] && $mE[$i]['E']<$mE[$i+1]['E'] && $mE[$i]['E']<$mE[$i-2]['E'] && $mE[$i]['E']<$mE[$i+2]['E'] && $mE[$i]['E']<$mE[$i-3]['E'] && $mE[$i]['E']<$mE[$i+3]['E']) 
  if($mE[$i]['E']<$mE[$i-1]['E'] && $mE[$i]['E']<$mE[$i+1]['E'] && $mE[$i]['E']<$mE[$i-2]['E'] && $mE[$i]['E']<$mE[$i+2]['E'] && $mE[$i]['E']<$mE[$i-3]['E'] && $mE[$i]['E']<$mE[$i+3]['E']) { 
   if(count($Extremum)>0) {
    ...
    $last_i = $last['index'];
    if(abs($mE[$last_i]['E']-$mE[$i]['E'])<2.0) continue;
    if($last['type']=='<font color=blue>min</font>') continue;
   }
...
// fprintf($hout,"  <img width=600 name=Eh id=Eh src=\"img/%s-eh.png\"><br/>\n",$Name);
 fprintf($hout,"  <img width=900 name=Eh id=Eh src=\"img/%s.svg\"><br/>\n",$Name);
